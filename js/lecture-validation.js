document.addEventListener('DOMContentLoaded', function () {
    ...
    // Completed toggle
    const completedKey = 'lecture_validation_completed_v1';
    ...
    render(localStorage.getItem(completedKey) === '1');
    ...
    const quizKey = 'lecture_validation_quiz_score_v1';
    const form = document.getElementById('quizForm');
    const result = document.getElementById('quizResult');
    const submit = document.getElementById('submitQuiz');
    const showStored = () => {
        const s = localStorage.getItem(quizKey);
        if (s && result) { result.textContent = `Previous score: ${s}/5`; result.classList.add('text-info'); }
    };
    showStored();
